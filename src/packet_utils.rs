...
/// Primitive data read write function
impl Buf {
    ...
    pub fn write_byte(&mut self, num: u8) {
...
impl Buf {
    pub fn write_packet_id(&mut self, id: u8) {
        self.write_var_i32(id.into());
    }
    ...
    pub fn write_var_u32_slice(&mut self, slice: &[u32]) {
        self.write_var_i32(slice.len() as i32);
        for i in slice {
            self.write_var_i32(*i as i32);
        }
    }
    ...
    pub fn read_var_u32_slice(&mut self) -> Vec<u32> {
        ...
        for _ in 0..length {
            nums.push(self.read_var_i32() as u32);
        }
        nums
    }
...
impl Buf {
    ...
    /// Byte shifter
    /// Moves the buffer to read by amount of byte
    /// - `distance` - Current positive byte + (distance)*byte
...
impl Buf {
    ...
    /// there is negative VarInt
    pub fn write_var_i32(&mut self, mut value: i32) {
        loop {
            let mut temp = (value as u8) & 0x7F;
            // Logical shift of the bits (cast to u32)
    ...
    pub fn write_var_i64(&mut self, mut value: i64) {
        loop {
            let mut temp = (value as u8) & 0x7F;
